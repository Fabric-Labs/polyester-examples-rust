//! Create a TWAP trigger, read it back, then cancel it.
...
async fn main() -> anyhow::Result<()> {
    ...
    let limit_price = resolve_post_only_buy_limit_price(&client, &symbol, pair.as_ref()).await?;
    let qty = min_base_qty_for_pair(pair.as_ref(), &limit_price)?;
    let client_trigger_id = unique_client_order_id("trg-twap");
    let duration_ms = 600_000;
    let slice_ms = 300_000;

    println!(
        "Creating TWAP trigger: symbol={symbol} side=buy qty={qty} limit={limit_price} \
         duration_ms={duration_ms} slice_ms={slice_ms}"
    );
    ...
    let created = client
        ...
        .create(CreateTriggerParams {
            ...
            trigger_price: None,
            limit_price: Some(Price::from_decimal_str(&limit_price, Some(symbol.clone()))?),
            ...
            twap_duration_ms: Some(duration_ms),
            twap_slice_interval_ms: Some(slice_ms),
